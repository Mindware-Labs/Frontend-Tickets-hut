...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(getBackendUrl(`/knowledge/${id}`));
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(getBackendUrl(`/knowledge/${id}/with-file`), {
      method: "PATCH",
      body: upstream,
    });
    const data = await response.json();
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          message: data?.message || "Failed to update knowledge",
        },
        { status: response.status }
      );
    }
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          message: data?.message || "Failed to delete knowledge",
        },
        { status: response.status }
      );
    }
