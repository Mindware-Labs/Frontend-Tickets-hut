...
export default function TicketsPage() {
  ...
  const filteredTickets = useMemo(() => {
    return mockTickets.filter(ticket => {
      const matchesSearch =
        ...
        ticket.campaign.toLowerCase().includes(search.toLowerCase())
  ...
  }, [search, statusFilter, priorityFilter, sortBy, sortOrder, activeView])
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Open": return "bg-blue-500 text-white shadow-blue-500/20"
      case "In Progress": return "bg-amber-500 text-white shadow-amber-500/20"
      case "Closed": return "bg-emerald-500 text-white shadow-emerald-500/20"
    ...
    <div className="h-[calc(100vh-theme(spacing.24))] flex flex-col lg:flex-row gap-6 animate-in fade-in duration-500">
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-hidden pt-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button className="btn-primary-modern shadow-lg shadow-primary/20">
            ...
            <Table>
              <TableHeader className="bg-muted/30 sticky top-0 z-10 backdrop-blur-md">
                <TableRow className="hover:bg-transparent border-b border-border/50">
                  ...
                  <TableHead className="font-bold text-xs uppercase tracking-wider cursor-pointer hover:text-primary transition-colors" onClick={() => toggleSort('clientName')}>
                    <div className="flex items-center gap-1">Client / Subject <ArrowUpDown className="h-3 w-3" /></div>
                  </TableHead>
                  <TableHead className="w-[150px] font-bold text-xs uppercase tracking-wider hidden md:table-cell">Campaign</TableHead>
                  ...
                  <TableHead className="w-[100px] font-bold text-xs uppercase tracking-wider hidden md:table-cell">Priority</TableHead>
                  <TableHead className="w-[120px] font-bold text-xs uppercase tracking-wider text-right pr-4">Created</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-[400px] text-center text-muted-foreground">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <Button variant="link" onClick={() => { setSearch(''); setStatusFilter('all'); setPriorityFilter('all') }}>Clear filters</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex flex-col py-0.5">
                          ...
                          <span className="text-[11px] text-muted-foreground line-clamp-1">Issue reported regarding {ticket.type} payment...</span>
                        </div>
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        <span className="text-xs text-muted-foreground truncate block max-w-[140px]">{ticket.campaign}</span>
                      </TableCell>
                      <TableCell>
                        {ticket.assignedTo ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-foreground/70 hidden lg:inline">{ticket.assignedTo.split(' ')[0]}</span>
                          </div>
                        ) : (
                          <span className="text-[10px] text-muted-foreground italic pl-1">Unassigned</span>
                        )}
                      </TableCell>
                      <TableCell>
                        <div className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border ${ticket.status === 'Open' ? 'border-primary/20 bg-primary/5 text-primary' : ticket.status === 'Closed' ? 'border-emerald-500/20 bg-emerald-500/5 text-emerald-600' : 'border-amber-500/20 bg-amber-500/5 text-amber-600'}`}>
                          {ticket.status}
                        </div>
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        {ticket.priority === 'High' && <Badge variant="destructive" className="text-[10px] h-5 px-1.5">High</Badge>}
                        {ticket.priority === 'Medium' && <Badge variant="secondary" className="text-[10px] h-5 px-1.5 bg-amber-500/10 text-amber-600 hover:bg-amber-500/20">Medium</Badge>}
                        {ticket.priority === 'Low' && <span className="text-[10px] text-muted-foreground">Low</span>}
                      </TableCell>
                      <TableCell className="text-right pr-4">
                        <span className="text-xs text-muted-foreground whitespace-nowrap">{new Date(ticket.createdAt).toLocaleDateString("en-US", { month: 'short', day: 'numeric' })}</span>
                      </TableCell>
                    </TableRow>
                  ))
                )}
            ...
            <>
              <SheetHeader className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">via Phone</span>
                  </div>
                  <Badge variant="secondary" className={`${getStatusColor(selectedTicket.status)}`}>{selectedTicket.status}</Badge>
                </div>
                <SheetTitle className="text-xl font-bold">{selectedTicket.clientName}</SheetTitle>
                <SheetDescription className="flex items-center gap-2">
                  <span>{selectedTicket.phone}</span>
                  <span>•</span>
                  <span>{selectedTicket.campaign}</span>
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="py-8 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button className="btn-primary-modern h-10">
                ...
                <div className="glass-card rounded-xl p-4 border border-border/50">
                  ...
                  <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <Avatar className="h-5 w-5">
                          <AvatarFallback className="text-[9px]">{selectedTicket.assignedTo ? selectedTicket.assignedTo.substring(0, 2).toUpperCase() : 'NA'}</AvatarFallback>
                        </Avatar>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm">{new Date(selectedTicket.createdAt).toLocaleString("en-US", { dateStyle: 'medium', timeStyle: 'short' })}</p>
                    </div>
                    <div className="space-y-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative pl-4 border-l-2 border-border/40 space-y-6">
                    <div className="relative">
                      ...
                      <div className="space-y-1">
                        <p className="text-sm"><span className="font-semibold">System</span> created this ticket</p>
                        <p className="text-xs text-muted-foreground text-[10px]">{new Date(selectedTicket.createdAt).toLocaleString("en-US")}</p>
                      </div>
                    </div>
                    <div className="relative">
                      <div className="absolute -left-[21px] top-1 h-3 w-3 rounded-full border-2 border-background bg-zinc-400"></div>
                      <div className="space-y-1">
                        <p className="text-sm"><span className="font-semibold">{selectedTicket.assignedTo || 'Agent'}</span> viewed details</p>
              ...
              <SheetFooter className="mt-auto">
                <Button variant="ghost" className="w-full text-rose-500 hover:text-rose-600 hover:bg-rose-500/10">Archive Ticket</Button>
              </SheetFooter>
            </>
          ) : null}
        </SheetContent>
